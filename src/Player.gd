...
@export var max_angle:float = 95.0
...
func _ready() -> void:
	...
	HUD = get_node("HUD")
	_on_screen_changed()
	get_viewport().size_changed.connect(_on_screen_changed)
...
func _walk(delta: float) -> void:
	...
	# interpolation
	temp_vel = temp_vel.lerp(direction*_speed, temp_accel*delta)
	...
	# Move
	get_floor_normal()
#	var moving = move_and_slide(vel, _snap, FLOOR_NORMAL, true, 4, FLOOR_MAX_ANGLE, false)
#	if is_on_wall():
#		vel = moving
#	else:
#		vel.y = moving.y
